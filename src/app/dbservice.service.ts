...
import { SQLite, SQLiteObject } from '@awesome-cordova-plugins/sqlite/ngx';// solucion
...
export class DbserviceService {
  ...
  private initDatabase(){
    this.sqlite.create({
      ...
      this.db = db;
      this.createTables();
      this.isBDReady.next(true);//Emitimos true cuando la base de datos esta lista
      this.presentToast('Base de datos y tabla creada con éxito');
  ...
  validarUsuario(usuario: string, password: string) {
    ...
    .then((res) => {
      if (res.rows.length > 0){
        return res.rows.item(0); //retorna el primer valor que coincide
      } else {
        return null;//Retorna null si no se encontro ningun usuario
      }
    })
